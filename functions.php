...
add_action('rest_api_init', 'add_tag_to_JSON');
function add_tag_to_JSON()
{
	register_rest_field(
		'post',
		'tags',
		array(
			'get_callback' => 'wp_get_tags_names',
			'update_callback' => null,
			'schema' => null,
		)
	);
...
function wp_get_tags_names($object, $field_name, $request)
{
	...
	foreach ($tags as $tag) {
		...
		$formatted_tags[$i]['tag_link'] = get_tag_link($tag->term_id);
		$formatted_tags[$i]['tag_name'] = $tag->name;
		$i++;
	}
